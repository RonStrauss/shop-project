...
export class AdminService {
  ...
  async getAllCategories() {
    const res = await fetch('http://localhost:1000/lists/categories', {
      credentials: 'include',
    });
  ...
  async editProduct(product: Product, id: string) {
    ...
    if (!data.err) {
      ...
      this._snackBar.open('Product has been edited', 'Ok', {
        duration: 4000,
      });
    } else {
      alert(data.msg);
  ...
  async addNewProduct(product: Product) {
    ...
    if (!data.err) {
      this._shop.setProducts(data);
      this._snackBar.open('New product successfully created', 'Ok', {
        duration: 4000,
      });
    } else {
      alert(data.msg);
